...
def main():
    ...
    # Cargar modelo
    try:
        model = load_model("yolov5s.pt")  # Ajusta la ruta según tu estructura
        ...
        if uploaded_video:
            ...
            try:
                with st.spinner("Procesando video... Esto puede tomar varios minutos."):
                    # Procesar video
                    results = detect_logo_in_video(video_path, "yolov5s.pt")
                    ...
                    if Path("results").exists():
                        video_file = Path("results") / f"detected_{Path(video_path).stem}.mp4"
